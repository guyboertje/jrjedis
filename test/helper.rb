...
JEDIS = JrJedis::Redis.new(OPTIONS)
REDIS_RUBY = Redis.new(OPTIONS)
...
module Helper
  ...
  module Generic
    ...
    def teardown
      @redis.quit if @redis && @redis.respond_to?(:quit)
    end
  ...
  module Client
    ...
    def _new_client(options = {})
      opts = _format_options(options).merge(:driver => ENV["conn"])
      JrJedis::Redis.new(opts)
    end
  end
  ...
  module Distributed
    ...
    def version
      Version.new(JEDIS.info.first["redis_version"])
    end
